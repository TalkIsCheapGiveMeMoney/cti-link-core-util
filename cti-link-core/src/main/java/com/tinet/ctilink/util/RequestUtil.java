...
public class RequestUtil {
	 ...
	 * <p>
	 * 数祖由同名参数的值组成， 例如将request中名为"longArry"的值转化成数组： <code><pre>
	...
	public Date getDate(String year, String month, String day) {
		try {
			Calendar cl = new GregorianCalendar(getInt(year), getInt(month), getInt(day));
			return cl.getTime();
		} catch (Exception e) {
		}
		return new Date();
	...
	/**
	 * 从request中获取参数，将参数解析成date对象返回。 如果name对应的参数值为空或者无法解析成日期，则返回null。
	 * 
	 * @param name
	 *            参数名称。
	 * @param format
	 *            格式字符串，{@link DateUtil#FMT_DATE_YYYY_MM_DD}等。
	 * @return
	 */
	...
	/**
	 * 从request中获取参数，将参数解析成Timestamp对象返回。 如果name对应的参数值为空或者无法解析成日期，则返回null。
	 * 
	 * @param name
	 *            参数名称。
	 * @param format
	...
	/**
	 * 从HttpServletRequest中得到boolean值。 如果参数不存在或者为空字符串返回false，否则返回true。
	 * 
	 * @param name
	 *            参数名
	 */
	...
	/**
	 * 将当前请求的URL进行url编码。 如果参数isEncode为true，则对当前的URL进行编码，否则直接返回URL。
	 * 
	 * @param isEncoode
	 *            是否编码的标志。
	 * @return
	...
	/**
	 * 将一个字符串进行编码。 例如中国被编码为"%D6%D0%B9%FA"。
	 * 
	 * @param s
	 *            源文。
	 * @return 编码后的码文。
