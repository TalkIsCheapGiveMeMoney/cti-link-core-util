...
public class RemoteClient {
	...
	public static String getIpAddr(HttpServletRequest request) {
		...
		if (ip != null && ip.length() > 0 && !"unknown".equalsIgnoreCase(ip)) {
			...
			Matcher m = pattern.matcher(ip.replaceAll("[ ,]", ""));
			//elb下ip形式为"221.123.160.250, 54.223.192.207"
			if (!m.matches()) {
				ip = "illegal";
			}
		}
	...
	public static String getKeyword(String refer) {
		String keywordReg = "(?:yahoo.+?[\\?|&]p=" + "|google.+?q=" + "|baidu.+?wd=" + "|soso.+?w=" + "|youdao.+?q=" + "|zhongsou.+?w=" + "|sogou.+?query=" + "|alltheweb.+?q=" + "|bing.+?q=)" + "([^&]*)";
