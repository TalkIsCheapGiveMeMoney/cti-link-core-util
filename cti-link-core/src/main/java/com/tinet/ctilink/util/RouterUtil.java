...
public class RouterUtil {
    ...
    public static Router getRouter (int enterpriseId, int routerClidCallType, Caller caller) {
        ...
        EnterpriseRouter enterpriseRouter = ContextUtil.getContext().getBean(RedisService.class).get(String.format(CacheKey.ENTERPRISE_ROUTER_ENTERPRISE_ID, enterpriseId)
                , EnterpriseRouter.class, Const.REDIS_DB_CONF_INDEX);
        Router router = null;
    ...
    public static Router getRouter(int routersetId, Caller caller) {
        ...
        List<Router> routerList = ContextUtil.getContext().getBean(RedisService.class)
                .getList(String.format(CacheKey.ROUTER_ROUTERSET_ID, routersetId), Router.class, Const.REDIS_DB_CONF_INDEX);
        //找prefix
    ...
    public static Gateway getRouterGateway(int enterpriseId, int routerClidCallType, Caller caller) {
        ...
        if (router != null) {
            List<Gateway> gatewayList = ContextUtil.getContext().getBean(RedisService.class)
                    .getList(CacheKey.GATEWAY, Gateway.class, Const.REDIS_DB_CONF_INDEX);
