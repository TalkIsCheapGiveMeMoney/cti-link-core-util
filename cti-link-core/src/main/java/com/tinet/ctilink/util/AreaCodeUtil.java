    ...
    public static Caller updateGetAreaCode(String number, String gateway) {
        ...
        if (Pattern.compile("^010").matcher(number).find()
            ...
            AreaCode areaCode = redisService.get(String.format(CacheKey.AREA_CODE_PREFIX, number.substring(2, 7)),
                    AreaCode.class, Const.REDIS_DB_AREA_CODE_INDEX);
            if (areaCode != null) {
...
//                    this.saveNewAreaCode(numberPrefix, areaCode, province, city);
            ...
            if(number.length() >=3 ){//大于3表示没有区号的固话，给一个区号，或者95555/10086/10010/110/112/999等号码都认为是本地固话
                if(!StringUtils.isEmpty(gateway)){
                    boolean find = false;
                    List<Gateway> gatewayList = redisService.getList(CacheKey.GATEWAY, Gateway.class, Const.REDIS_DB_CONF_INDEX);
                    ...
                    if(find == false){
                        SystemSetting systemSetting = redisService.get(String.format(CacheKey.SYSTEM_SETTING_NAME
                                , Const.SYSTEM_SETTING_NAME_DEFAULT_AREA_CODE), SystemSetting.class, Const.REDIS_DB_CONF_INDEX);
